...
| Colden Baxter (U Idaho) & students | Beaver Creek, Monumental Creek, Rush Creek (all near their Big Creek confluences) | Confirmed as part of your Big Creek trip — see the sample spots below for each. Rush Creek beyond its near-confluence spot is remote and difficult, so realistically just the one sample there. |
...
**Rush Creek and (Big Creek's) Beaver Creek**: both have an easily reachable sample spot right near their mouths (Rush Creek: \~45.088°N, -114.882°W; Beaver Creek: \~45.173°N, -115.245°W) — everything further upstream on either is remote and not worth detouring for, so just the one sample from each. Neither sub-tributary is otherwise covered by anyone this season, so these two are a real addition to the dataset, not a bonus.
